...
// CORREÇÃO: Removida referência ambígua `usuarios(id, nome_completo)` da query.
// ordens_servico agora tem 3 FKs para usuarios (mecanico_id, aberto_por,
// finalizado_por), então o Supabase exige FK hints explícitos em todas as
// referências. Mantida apenas `aberto_usuario:usuarios!aberto_por` que é
// suficiente para exibir o nome de quem abriu a O.S.
...
function CardOS({ os, index, onClick }) {
  ...
  const semAtendimento = !os.mecanico_id && os.status === 'em_andamento';

  // FK hint garante nome correto: aberto_usuario via !aberto_por
  ...
  return (
    ...
    >
      ...
      <div style={S.cardBot}>
        <span style={S.metaItem}><UserIcon /> Solic.: {os.solicitante}</span>
        <span style={S.metaDot} />
        <span style={S.metaItem}><CalendarIcon /> {formatarDataHora(os.inicio_em)}</span>
        {duracao && (
          <><span style={S.metaDot} /><span style={S.metaItem}><TimerIcon /> {duracao}</span></>
        )}
      </div>

      {/* Linha 4: Aberto por + atualização */}
      <div style={S.mecRow}>
        <MecIcon />
        <span style={S.mecNome}>Aberto por: {abertoPorNome}</span>
        {isFinalizada && os.atualizado_em && (
          <span style={S.atualizadoTag}>Atualizado {tempoRelativo(os.atualizado_em)}</span>
        )}
        <ChevronIcon />
      </div>
    </article>
...
export default function ListagemCorretivas() {
  ...
  const fetchOrdens = useCallback(async () => {
    ...
    try {
      ...
      const { data: todos } = await supabase.from('ordens_servico').select('status');
      if (todos) {
...
function QueueSmIcon() { return <svg width="10" height="10" viewBox="0 0 24 24" fill="none" style={{ flexShrink:0 }}><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/><circle cx="9" cy="7" r="4" stroke="currentColor" strokeWidth="2"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>; }
...
const S = {
  ...
  contadoresRow: { display:'flex', alignItems:'center', marginBottom:'16px', backgroundColor:'#F8FAFC', border:'1px solid #E8EDF2', borderRadius:'10px', padding:'12px 0', overflow:'hidden' },
  ...
  filaTag:       { display:'flex', alignItems:'center', gap:'4px', padding:'2px 7px', borderRadius:'20px', backgroundColor:'rgba(245,158,11,0.12)', color:'#92400E', fontSize:'10px', fontWeight:'700', border:'1px solid rgba(245,158,11,0.3)', flexShrink:0 },
