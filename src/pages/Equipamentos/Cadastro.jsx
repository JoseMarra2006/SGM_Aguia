...
export default function Cadastro() {
  ...
  // PDF do manual: { file: File, nome: string } | null
  ...
  const handleAddImagens = (e) => {
    ...
    setImagens((prev) => {
      const combined = [...prev, ...validas];
      return combined.slice(0, MAX_IMAGENS);
    });

    // Reseta o input para permitir reselecionar os mesmos arquivos
    e.target.value = '';
  };
  ...
  const handleAddPDF = (e) => {
    ...
    if (f.type !== 'application/pdf') {
      setErros((p) => ({ ...p, manual: 'Apenas arquivos PDF são aceitos.' }));
      return;
    }
    if (f.size > MAX_PDF_MB * 1024 * 1024) {
      setErros((p) => ({ ...p, manual: `O PDF excede o limite de ${MAX_PDF_MB}MB.` }));
      return;
    }
    setErros((p) => ({ ...p, manual: null }));
    setManual({ file: f, nome: f.name });
    e.target.value = '';
  };
  ...
  const uploadArquivo = async (bucket, file, path) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (let i = 0; i < imagens.length; i++) {
        ...
        const url = await uploadArquivo('equipamentos-imagens', imagens[i].file, path);
        urlsImagens.push(url);
      }

      // 2. Upload do manual PDF
      let manualUrl = null;
      if (manual) {
        setProgresso('Enviando manual PDF...');
        const path = `${profile.id}/${Date.now()}_manual.pdf`;
        manualUrl = await uploadArquivo('manuais', manual.file, path);
      }

      // 3. Insere o equipamento no banco
      setProgresso('Salvando equipamento...');
      const { data: equipamentoCriado, error: errEq } = await supabase
        .from('equipamentos')
        .insert({
          nome: nome.trim(),
      ...
      // 5. Redireciona para os detalhes do equipamento criado
      setProgresso('Concluído!');
      navigate(`/equipamentos/${equipamentoCriado.id}`, { replace: true });
  ...
  return (
    <div style={S.page}>
      ...
      <main style={S.main}>
        ...
        {/* ── Bloco: Manual PDF ── */}
        <FormSection title="Manual PDF (opcional)" icon={<PdfIcon />}>
          <input
            ref={pdfInputRef}
            type="file"
          ...
          {manual ? (
            <div style={S.pdfSelected}>
              <div style={S.pdfSelectedInfo}>
                <PdfIcon />
                <span style={S.pdfNome}>{manual.nome}</span>
              </div>
              <button
                onClick={() => setManual(null)}
                style={S.pdfRemoveBtn}
                disabled={enviando}
              >
                Remover
              </button>
          ...
          {erros.manual && <span style={S.fieldError}>{erros.manual}</span>}
          <p style={S.hint}>Máx. {MAX_PDF_MB}MB. Será acessível na tela de detalhes.</p>
        </FormSection>
...
const CSS = `
  ...
  input:focus, textarea:focus { outline: none; border-color: #0F4C81 !important; box-shadow: 0 0 0 3px rgba(15,76,129,0.1) !important; }
`;
...
const S = {
  ...
  input: {
    ...
    backgroundColor: '#FAFBFC', color: '#0D1B2A',
  ...
  uploadBtn: {
    ...
    border: '1.5px dashed #CBD5E1', borderRadius: '9px',
    backgroundColor: '#F8FAFC', color: '#64748B',
    fontSize: '13px', fontWeight: '600', cursor: 'pointer',
    fontFamily: 'inherit', transition: 'border-color 0.15s, background 0.15s',
  },
  // PDF
  pdfSelected: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', backgroundColor: '#FFF5F5', border: '1.5px solid #FECACA', borderRadius: '9px' },
  pdfSelectedInfo: { display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden' },
  pdfNome: { fontSize: '13px', color: '#0D1B2A', fontWeight: '500', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  pdfRemoveBtn: { background: 'none', border: 'none', color: '#EF4444', fontSize: '12px', fontWeight: '600', cursor: 'pointer', flexShrink: 0, fontFamily: 'inherit' },
  // Peças
