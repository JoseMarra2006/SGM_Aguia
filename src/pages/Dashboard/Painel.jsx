...
// CORREÇÃO (Fila Aberta — Conflito #3):
//   • fetchListas: removido filtro `qOS.eq('mecanico_id', profile.id)`
//     Com fila aberta, mecânicos devem ver TODAS as OS em andamento,
//     não só as designadas a eles (que agora são sempre null).
//   • fetchListas: removido filtro equivalente de preventivas também mantido
      ...
      <div style={NS.painel}>
        <div style={NS.header}>
          <div style={NS.headerLeft}>
            ...
            {unreadCount > 0 && <span style={NS.unreadBadge}>{unreadCount}</span>}
          </div>
          <div style={NS.headerAcoes}>
            ...
            <button onClick={onClose} style={NS.btnFechar}><CloseIcon /></button>
          </div>
        </div>

        <div style={NS.lista}>
          {notifLoading ? (
            <div style={NS.vazio}><div style={NS.spinner} /></div>
  ...
  return (
    <div onClick={onClick} style={S.osItem} role="button" tabIndex={0}>
      <div style={S.osItemLeft}>
        ...
        <div style={S.osMeta}>
          <span style={S.osMetaItem}><UserSmIcon /> {abertoPorNome}</span>
...
export default function Painel() {
  ...
  useEffect(() => {
    ...
    return () => { if (unsubNotifRef.current) unsubNotifRef.current(); };
    ...
    async function fetchListas() {
      ...
      try {
        ...
        const qOS = supabase
          .from('ordens_servico')
          .select(`
            id, problema, inicio_em, aberto_por,
            equipamentos(nome),
            aberto_usuario:usuarios!aberto_por(nome_completo),
        ...
        <div style={S.headerInner}>
          ...
          <div style={S.headerAcoes}>
            ...
            <button onClick={() => setNotifPanelOpen(true)} style={S.btnSino} title="Notificações"
              ...
              <BellNavIcon />
              {unreadCount > 0 && (
                <span style={S.sinoBadge}>{unreadCount > 9 ? '9+' : unreadCount}</span>
              )}
            </button>
      ...
      <main style={S.main}>
        ...
        <section>
          ...
          <div style={S.metricasGrid}>
            <CardMetrica index={0} label="Equipamentos" valor={metricas.equipamentos}
            ...
            <CardMetrica index={1} label="Em manutenção" valor={metricas.emManutencao}
            ...
            <CardMetrica index={2} label="OS abertas" valor={metricas.osAbertas}
            ...
            <CardMetrica index={3} label="Prev. atrasadas" valor={metricas.prevAtrasadas}
          ...
          {loadingListas ? (
            ...
            <div style={S.listaCard}>
              {prevAtrasadas.map(ag => (
                <ItemPrevAtrasada key={ag.id} ag={ag}
...
// ─── Estilos Painel de Notificações ──────────────────────────
const NS = {
  overlay:       { position: 'fixed', inset: 0, zIndex: 40, backgroundColor: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(2px)' },
  painel:        { position: 'fixed', top: 0, right: 0, bottom: 0, zIndex: 50, width: 'min(380px, 100vw)', backgroundColor: '#FFFFFF', display: 'flex', flexDirection: 'column', boxShadow: '-8px 0 32px rgba(0,0,0,0.15)', animation: 'slideInRight 0.25s ease both' },
  header:        { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 12px', borderBottom: '1px solid #E8EDF2', gap: '8px', flexShrink: 0 },
  ...
  spinner:       { width: '24px', height: '24px', border: '3px solid #E8EDF2', borderTopColor: '#20643F', borderRadius: '50%', animation: 'shimmer 1s linear infinite' },
  item:          { width: '100%', display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '14px 16px', border: 'none', borderBottom: '1px solid #F1F5F9', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'background-color 0.15s' },
  ...
  itemMsg:       { fontSize: '12px', color: '#64748B', lineHeight: 1.45, overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' },
  itemTempo:     { fontSize: '11px', color: '#94A3B8', fontWeight: '500' },
  dotNaoLida:    { width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#20643F', flexShrink: 0, marginTop: '5px' },
};
