...
// ADIÇÕES v2 (admin CRUD):
//   • Botão de exclusão (lixeira) em CardPeca — visível APENAS para superadmin
//   • ModalConfirmarExclusaoPeca — confirmação antes de deletar do estoque
//   • handleExcluirPeca — deleta da tabela pecas_oficina
// INALTERADO: lógica de edição (ModalPeca), métricas, busca e filtros.
    ...
    <div style={S.modalOverlay} onClick={onClose}>
      <div style={S.modalBox} onClick={e => e.stopPropagation()}>
        <div style={S.modalHeader}>
          ...
          <button onClick={onClose} style={S.modalCloseBtn} disabled={salvando}><CloseIcon /></button>
        </div>
        ...
        <div style={S.modalCorpo}>
          <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
            ...
            <input type="text" value={nome} onChange={e => setNome(e.target.value)}
              ...
              placeholder="Ex: Rolamento 6205 ZZ" maxLength={100} disabled={salvando} autoFocus />
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
            ...
            <div style={S.qtdRow}>
              <button onClick={() => setQtd(v => Math.max(0, v-1))} style={S.qtdBtn}
                disabled={salvando || qtd <= 0}>−</button>
              <input type="number" min={0} value={qtd}
                ...
                disabled={salvando} />
              <button onClick={() => setQtd(v => v+1)} style={S.qtdBtn} disabled={salvando}>+</button>
            </div>
...
function ModalConfirmarExclusaoPeca({ peca, excluindo, onClose, onConfirmar }) {
  if (!peca) return null;
  return (
    <div style={S.modalOverlay} onClick={() => !excluindo && onClose()}>
      <div style={{ ...S.modalBox, ...S.modalBoxCentrado }} onClick={e => e.stopPropagation()}>
        <div style={S.exclusaoIcone}><TrashIcon size={28} cor="#EF4444" /></div>
        <h3 style={S.exclusaoTitulo}>Excluir peça?</h3>
        <p style={S.exclusaoSubtitulo}>
          <strong>"{peca.nome}"</strong> será removida permanentemente do estoque.
          {peca.quantidade_estoque > 0 && (
            <><br /><span style={{ color:'#F59E0B', fontWeight:600 }}>
              ⚠ Estoque atual: {peca.quantidade_estoque} unidade(s).
            </span></>
          )}
          <br /><span style={{ color:'#EF4444', fontWeight:600 }}>Esta ação não pode ser desfeita.</span>
        </p>
        <div style={S.exclusaoBotoes}>
          <button onClick={onClose} style={S.exclusaoBtnCancelar} disabled={excluindo}>Cancelar</button>
          <button onClick={onConfirmar} disabled={excluindo}
            style={{ ...S.exclusaoBtnExcluir, opacity: excluindo ? 0.7 : 1 }}>
            {excluindo ? <><Spinner /> Excluindo...</> : 'Sim, excluir'}
          </button>
        </div>
      </div>
    </div>
...
// ─── Card de Peça ─────────────────────────────────────────────
function CardPeca({ peca, index, isSuperAdmin, onEditar, onExcluir }) {
  ...
  return (
    <div style={{ ...S.cardPeca, animationDelay: `${index * 45}ms` }}>
      <div style={S.cardPecaLeft}>
        <div style={{
          ...S.estoqueCirculo,
          backgroundColor: semEstoque ? 'rgba(239,68,68,0.1)' : estoqueB ? 'rgba(245,158,11,0.1)' : 'rgba(16,185,129,0.1)',
          border: `1px solid ${semEstoque ? 'rgba(239,68,68,0.25)' : estoqueB ? 'rgba(245,158,11,0.25)' : 'rgba(16,185,129,0.25)'}`,
        }}>
      ...
      {/* Ações — somente superadmin */}
      {isSuperAdmin ? (
        <div style={S.acoesCard}>
          <button onClick={() => onEditar(peca)} style={S.btnAcaoCard} title="Editar peça">
            <EditIcon />
          </button>
          <button onClick={() => onExcluir(peca)}
            style={{ ...S.btnAcaoCard, ...S.btnAcaoDelete }} title="Excluir peça">
            <TrashIcon size={13} cor="#FFFFFF" />
          </button>
        </div>
      ) : (
        <button onClick={() => onEditar(peca)} style={S.editBtn}><EditIcon /></button>
      )}
    </div>
  );
}
...
export default function Pecas() {
  ...
  const handleExcluir = async () => {
    ...
    try {
      ...
      setErroExclusao(`Erro ao excluir: ${err.message}`);
      console.error('[Pecas] Exclusão:', err.message);
    } finally {
      setExcluindo(false);
    }
  };
    ...
    <div style={S.page}>
      ...
      <main style={S.main}>
        ...
        <div style={S.buscaWrapper}>
          <SearchIcon />
          <input type="text" placeholder="Buscar peça..." value={busca}
            onChange={e => setBusca(e.target.value)} style={S.buscaInput} />
        ...
        <div style={S.filtrosRow}>
          {[
            ...
            <button key={f.v} onClick={() => setFiltroEstoque(f.v)}
          ...
          <div style={S.gridPecas}>
            {filtradas.map((p, i) => (
              <CardPeca
                ...
                onEditar={peca => setModalPeca({ peca })}
                onExcluir={peca => { setErroExclusao(''); setModalExcluir(peca); }}
              />
            ))}
          </div>
        )}

        {erroExclusao && <div style={S.erroToast}>{erroExclusao}</div>}
      </main>
...
const S = {
  ...
  // Ações do card
  acoesCard:    { display:'flex', gap:'4px', flexShrink:0 },
  btnAcaoCard:  { display:'flex', alignItems:'center', justifyContent:'center', width:'32px', height:'32px', border:'1.5px solid #E2E8F0', borderRadius:'8px', background:'#F8FAFC', cursor:'pointer', color:'#64748B', flexShrink:0 },
  btnAcaoDelete:{ backgroundColor:'#EF4444', borderColor:'#EF4444', color:'#FFFFFF' },
  ...
  erroToast:    { position:'fixed', bottom:'24px', left:'50%', transform:'translateX(-50%)', backgroundColor:'#FEF2F2', border:'1px solid #FECACA', borderRadius:'10px', padding:'12px 20px', fontSize:'13px', color:'#DC2626', zIndex:200, boxShadow:'0 4px 16px rgba(0,0,0,0.12)' },
  // Modal base
  ...
  modalBox:     { backgroundColor:'#FFFFFF', borderRadius:'20px 20px 0 0', width:'100%', maxWidth:'640px', overflow:'hidden', animation:'slideUp 0.25s ease' },
  modalBoxCentrado: { borderRadius:'16px', maxWidth:'380px', padding:'32px 28px', display:'flex', flexDirection:'column', alignItems:'center', gap:'14px', boxShadow:'0 20px 50px rgba(0,0,0,0.2)', animation:'fadeIn 0.2s ease', alignSelf:'center' },
  ...
  // Exclusão centrada
  exclusaoIcone:  { width:'60px', height:'60px', borderRadius:'50%', backgroundColor:'rgba(239,68,68,0.1)', display:'flex', alignItems:'center', justifyContent:'center' },
  exclusaoTitulo: { margin:0, fontSize:'18px', fontWeight:'800', color:'#0D1B2A', textAlign:'center' },
  exclusaoSubtitulo: { margin:0, fontSize:'13px', color:'#64748B', textAlign:'center', lineHeight:1.6 },
  exclusaoBotoes:   { display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px', width:'100%' },
  exclusaoBtnCancelar: { padding:'12px', backgroundColor:'#F8FAFC', color:'#64748B', border:'1.5px solid #E2E8F0', borderRadius:'9px', fontSize:'13px', fontWeight:'600', cursor:'pointer', fontFamily:'inherit' },
  exclusaoBtnExcluir: { display:'flex', alignItems:'center', justifyContent:'center', padding:'12px', backgroundColor:'#EF4444', color:'#FFFFFF', border:'none', borderRadius:'9px', fontSize:'13px', fontWeight:'700', cursor:'pointer', fontFamily:'inherit' },
};
