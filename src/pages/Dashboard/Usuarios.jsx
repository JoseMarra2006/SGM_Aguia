...
//   • ModalEditarUsuario: edita nome_completo, role, cpf, rg, nome_mae
//   • ModalExcluirUsuario: confirmação antes de deletar de public.usuarios
//   • Botões ✏ editar e 🗑 excluir em cada LinhaUsuario (somente superadmin)
//   • INALTERADO: ModalCadastro, lógica de fetch, paleta verde, layout
  ...
  const handleCadastrar = async () => {
    ...
    try {
      const { data, error: fnError } = await supabase.functions.invoke('create-user', {
        body: { email: email.trim().toLowerCase(), password: senha, nome_completo: nome.trim(), cpf: cpf.replace(/\D/g, ''), rg: rg.trim() || null, nome_mae: nomeMae.trim() || null, role },
      });
      ...
      <div style={S.modalBox} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.modalCorpo}>
          <div style={S.toggleRow}>
            {[{ v: 'mecanico', l: 'Mecânico' }, { v: 'superadmin', l: 'SuperAdmin' }].map(r => (
              <button key={r.v} onClick={() => setRole(r.v)} disabled={salvando}
                style={{ ...S.toggleBtn, ...(role === r.v ? S.toggleBtnAtivo : {}) }}>{r.l}</button>
            ))}
          </div>
          ...
          <div style={S.doisCols}>
            <FormField label="RG"><input type="text" value={rg} onChange={e => setRG(e.target.value)} style={S.input} placeholder="0000000" maxLength={20} disabled={salvando} /></FormField>
            <FormField label="Nome da mãe"><input type="text" value={nomeMae} onChange={e => setNomeMae(e.target.value)} style={S.input} placeholder="Nome completo" maxLength={80} disabled={salvando} /></FormField>
          </div>
          ...
          <div style={S.senhaHint}>🔒 O usuário será obrigado a trocar a senha no primeiro acesso.</div>
          {erroGlobal && <div style={S.erroGlobal}><AlertIcon /><span>{erroGlobal}</span></div>}
        </div>
    ...
    <div style={S.modalOverlay} onClick={onClose}>
      <div style={S.modalBox} onClick={e => e.stopPropagation()}>
        ...
        <div style={S.modalCorpo}>
          ...
          <FormField label="Nome completo *" error={erros.nome}>
            <input type="text" value={nome} onChange={e => setNome(e.target.value)} style={{ ...S.input, ...(erros.nome ? S.inputErr : {}) }} maxLength={80} disabled={salvando} />
          </FormField>
          <FormField label="CPF *" error={erros.cpf}>
            <input type="text" inputMode="numeric" value={cpf} onChange={e => setCPF(formatarCPF(e.target.value))} style={{ ...S.input, ...(erros.cpf ? S.inputErr : {}) }} placeholder="000.000.000-00" disabled={salvando} />
          </FormField>
          <div style={S.doisCols}>
            <FormField label="RG"><input type="text" value={rg} onChange={e => setRG(e.target.value)} style={S.input} maxLength={20} disabled={salvando} /></FormField>
            <FormField label="Nome da mãe"><input type="text" value={nomeMae} onChange={e => setNomeMae(e.target.value)} style={S.input} maxLength={80} disabled={salvando} /></FormField>
          </div>
          <div style={S.senhaHint}>ℹ️ Para alterar e-mail ou senha, use o painel do Supabase.</div>
          {erroGlobal && <div style={S.erroGlobal}><AlertIcon /><span>{erroGlobal}</span></div>}
        </div>
        <div style={S.modalFooter}>
          <button onClick={onClose} style={S.btnSecundario} disabled={salvando}>Cancelar</button>
...
function ModalExcluirUsuario({ usuario, onClose, onExcluido }) {
  ...
  const handleExcluir = async () => {
    ...
    try {
      // Remove de public.usuarios (RLS: superadmin).
      // O auth.users permanece como órfão — o usuário perde acesso
      // pois _loadProfile falhará no próximo login.
  ...
  return (
    <div style={S.modalOverlay} onClick={onClose}>
      <div style={{ ...S.modalBox, padding: '32px 28px', alignItems: 'center', gap: 14, display: 'flex', flexDirection: 'column' }}
        ...
        <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: '#0D1B2A' }}>Excluir usuário?</h3>
        ...
        <div style={{ width: '100%', padding: '11px 13px', backgroundColor: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 9, fontSize: 12, color: '#92400E', fontWeight: 500 }}>
          ⚠️ O.S. e checklists deste usuário serão preservados no histórico.
        </div>
        {erro && (
          <div style={{ width: '100%', padding: '11px 13px', backgroundColor: '#FEF2F2', border: '1px solid #FECACA', borderRadius: 9, fontSize: 12, color: '#DC2626' }}>{erro}</div>
        )}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, width: '100%', marginTop: 4 }}>
          <button onClick={onClose} disabled={excluindo}
            style={{ padding: 12, backgroundColor: '#F8FAFC', color: '#64748B', border: '1.5px solid #E2E8F0', borderRadius: 9, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            Cancelar
          </button>
          <button onClick={handleExcluir} disabled={excluindo}
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 12, backgroundColor: '#EF4444', color: '#FFFFFF', border: 'none', borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', opacity: excluindo ? 0.7 : 1 }}>
    ...
    <div style={{ ...S.linhaUsuario, animationDelay: `${index * 50}ms` }}>
      ...
      <div style={S.usuarioDir}>
        ...
        <div style={{ display: 'flex', gap: 4, marginTop: 5, justifyContent: 'flex-end' }}>
          ...
          <button onClick={() => onExcluir(usuario)} style={{ ...S.acaoBtn, borderColor: 'rgba(239,68,68,0.3)', color: '#EF4444' }} title="Excluir usuário">
            <TrashIcon size={14} />
          </button>
        </div>
      </div>
...
export default function Usuarios() {
  ...
  return (
    <div style={S.page}>
      ...
      {modalAberto && (
        <ModalCadastro onClose={() => setModalAberto(false)} onSucesso={() => { setModalAberto(false); fetchUsuarios(); }} />
      )}
      {modalEditar && (
        <ModalEditarUsuario usuario={modalEditar} onClose={() => setModalEditar(null)} onSucesso={() => { setModalEditar(null); fetchUsuarios(); }} />
      )}
      {modalExcluir && (
        <ModalExcluirUsuario usuario={modalExcluir} onClose={() => setModalExcluir(null)} onExcluido={() => { setModalExcluir(null); fetchUsuarios(); }} />
      )}

      <header style={S.topbar}>
        <button onClick={() => navigate('/dashboard')} style={S.backBtn}><BackIcon /></button>
        <h1 style={S.topbarTitulo}>Usuários</h1>
        <button onClick={() => setModalAberto(true)} style={S.btnNovoHeader}><PlusIcon /> Novo</button>
      </header>
...
const skBg = { background: 'linear-gradient(90deg,#F0F4F8 25%,#E8EDF2 50%,#F0F4F8 75%)', backgroundSize: '400px', animation: 'shimmer 1.4s infinite linear' };
